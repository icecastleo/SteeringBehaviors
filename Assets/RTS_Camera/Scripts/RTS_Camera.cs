    ...
    public class RTS_Camera : MonoBehaviour
    {
        ...
        private void Rotation()
        {
            if(useKeyboardRotation)
                transform.Rotate(Vector3.forward, RotationDirection * Time.deltaTime * rotationSped, Space.World);

            if (useMouseRotation && Input.GetKey(mouseRotationKey))
                m_Transform.Rotate(Vector3.forward, -MouseAxis.x * Time.deltaTime * mouseRotationSpeed, Space.World);
        }
